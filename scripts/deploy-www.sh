...
on_error() {
  ...
  log "${dim}Run with --inputs set; deployment modes also require AWS CLI credentials and .envrc values.${reset}"
}
...
Options:
  --inputs PATH   Product-owned website input JSON exported by
                  scripts/export-website-inputs.py in the product repo.
...
if [[ -z "${inputs_path}" ]]; then
  die "Set --inputs PATH or WEBSITE_INPUTS_PATH to the product website-inputs JSON."
fi
...
try:
    ...
    raise SystemExit(f"Invalid website inputs {path}: {err}")
...
if key == "productVersion":
    ...
    if not isinstance(value, str) or not re.fullmatch(r"[0-9]+\.[0-9]+\.[0-9]+([.-][0-9A-Za-z.-]+)?", value):
        raise SystemExit(f"Invalid productVersion in {path}: {value!r}")
    print(value)
elif key == "scannedPackageCount":
    value = data.get(key)
    if not isinstance(value, int) or value <= 0:
        raise SystemExit(f"Invalid scannedPackageCount in {path}: {value!r}")
    print(value)
else:
